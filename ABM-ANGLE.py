...
marketDF = pd.read_excel('data/XAUUSDHourlyIndicators.xlsx')
df = pd.read_excel('data/XAUUSDNewsSA[AnglE].xlsx')
daily_marketDF = pd.read_excel('data/indicatorsXAUUSDdaily.xlsx')
    ...
    for i in range(0, 5):
        ...
        if len(subDF):
            for dt, row in subDF.iterrows():
                embed = getNewsVector(row, 'clsBodyEmbed', ' ')
