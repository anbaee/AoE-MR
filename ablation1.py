...
marketDF = pd.read_excel('data/EURUSDHourlyIndicators.xlsx')
df = pd.read_excel('data/EURUSDNewsSA[AnglE].xlsx')
daily_marketDF = pd.read_excel('data/indicatorsEURUSDdaily.xlsx')
...
daily_marketDF['target'] = daily_marketDF['Close'].shift(-FUTURE_PERIOD_PREDICT)
...
def normalization(df):
    #,  'on_balance_volume' ,  'ATR','Volume','ADI',  'on_balance_volume' ,  'ATR','Volume','ADI',s
    df.drop(['High', 'Low', 'Open'], axis=1, inplace=True)
    drop_list = ['Close', 'EMA', 'bb_bbl',  'bb_bbh', 'bb_bbm',
                 'stochastic', 'RSI',  'momentum', 'MACD', 'wiliams', ]
...
#for i in range(len(train_news_x)):
#    train_news_x[i]+= 1000
#for i in range(len(validation_news_x)):
#    validation_news_x+= 1000
#for i in range(len(test_news_x)):
#    test_news_x[i]+= 1000

history, model = predictiveModel.ablationWithoutHourlyModelTrain (  train_daily, train_news_x, train_y
                                 ,validation_daily,  validation_news_x,  validation_y,
                                 problem='ablation1')
...
def computeAccuracy(df):
    ...
    pred = classify(df['predict'], df['predict'].iloc[1:])
    acc = accuracy_score(label[1:], pred)
    print('acc:', acc)
...
# plot_loss(history)
pred_train = model.predict([train_daily, train_news_x ])

pred_valid = model.predict([validation_daily, validation_news_x ])

pred_test = model.predict([test_daily, test_news_x])
print("Validation loss : {}".format(model.evaluate([validation_daily, validation_news_x ], np.array(validation_y))))
...
print("Test loss : {}".format(model.evaluate([test_daily, test_news_x ], np.array(test_y))))
