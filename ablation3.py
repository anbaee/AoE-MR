...
df = pd.read_excel('data/BTCUSDTNewsSA[AnglE].xlsx')
...
def normalization(df):
    # ,  'on_balance_volume' ,  'ATR','Volume','ADI',
    df.drop(['High', 'Low', 'Open'], axis=1, inplace=True)
    drop_list = ['Close', 'EMA', 'bb_bbl', 'on_balance_volume' ,  'ATR','Volume','ADI','bb_bbh', 'bb_bbm',
...
history, model = predictiveModel.ablationWithoutNews(train_x, train_daily, train_y
                                 , validation_x, validation_daily, validation_y,
                                                     problem='ablation3')
...
# plot_loss(history)
pred_train = model.predict([train_x,  train_daily])

pred_valid = model.predict([validation_x,  validation_daily])

pred_test = model.predict([test_x,  test_daily])
print("Validation loss : {}".format(
    model.evaluate([validation_x,  validation_daily], np.array(validation_y))))
...
print("Test loss : {}".format(model.evaluate([test_x,  test_daily], np.array(test_y))))
